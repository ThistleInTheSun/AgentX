"""拉取 arXiv 跨学科论文，并用 LLM 选出最有趣的一篇。"""
import html
import logging
...
def fetch_papers() -> list[Paper]:
    """返回 arXiv 上最新的一批跨学科论文。"""
    url = _build_query()
    log.info("拉取 arXiv 跨学科 feed ...")
...
def pick_paper(papers: list[Paper], is_processed) -> Paper | None:
    """先过滤未处理/英文/长度，再用 LLM 打分选出最有趣的一篇。"""
    candidates = []
    for paper in papers:
    ...
    log.info("从 %d 篇候选中挑选最有趣的一篇 ...", len(candidates))
    return explain.pick_most_interesting(candidates)

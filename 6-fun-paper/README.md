...
每天自动从 arXiv 跨学科领域搜刮一篇「有意思、适合大众阅读」的论文，生成一篇轻松有趣的中文解读草稿，输出到公众号草稿箱或本地 Markdown。

- 内容源：arXiv 跨学科分类（生物、物理、人机交互、天文、复杂系统等）
- 输出：优先写入公众号草稿箱；未配置公众号凭证时落盘到 `drafts/*.md`
- 解读风格：轻松、好奇、讲人话，突出「这篇论文在聊什么」「最有趣的点」「跟我有什么关系」
...
参考 `4daily-gloss` 的 `daily_run.sh`，把路径改为 `/mnt/d/xq/AgentX/6-fun-paper` 即可。
...
src/
  config.py       # .env 加载与常量
  fetch.py        # 拉取跨学科论文并用 LLM 筛选「最有趣」的一篇
  explain.py      # LLM 解读：轻松讲人话 + 有趣点挖掘
